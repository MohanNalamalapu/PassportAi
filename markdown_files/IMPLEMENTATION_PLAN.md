...
Handles file parsing with checks on environment-configurable file size limits (default 5MB, up to 10MB maximum), image MIME types (`image/jpeg` and `image/png` only), and filters out corrupted files.
...
Draws the photo using canvas, applies lighting filters, and formats downloads (JPEG, PNG, PDF export via `pdf-lib`). Sets up the results screen.
